<!DOCTYPE html>
      ...
      <body>
            ...
            <header>
                ...
                <h1 class="wrap-text">Laporan Barang Masuk</h1>
            </header>
        ...
        <tr>
            @foreach ($cetakBarangMasuk as $index => $barangMasuk)
            @if ($barangMasuk->BarangMasukDetail)
                @foreach ($barangMasuk->BarangMasukDetail as $barangMasukDetail)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{$barangMasuk->tanggal_masuk}}</td>
                        <td>{{$barangMasuk->Supplier->nama_supplier}}</td>
                        <td>{{ $barangMasukDetail->Barang->nama_barang }}</td>
                        <td>{{ $barangMasukDetail->jumlah }}</td>
                        <td>{{ $barangMasukDetail->satuan }}</td>
                        <td>Rp {{ number_format($barangMasukDetail->harga, 2,',', '.' )  }} / Dus</td>
                    </tr>
                @endforeach
            @endif
        @endforeach
